...
class ConfigCli {
  ...
  async init(vorpal, argv) {
    ...
    vorpal.command('jwt decode <token>')
      ...
      .option('-s --secret <issuer>', 'token secret')
      .description('Decode a display a jwt token.  if issuer and secret are passed, token will be verified first')
...
`);
  ...
  async jwtDecode(args) {
    ...
    if( args.options.issuer && args.options.secret ) {
      let issuer = args.options.issuer;
      ...
      Logger.log('Not verified. Provide issuer and secret to verify.');
    }
