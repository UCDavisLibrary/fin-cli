...
class FileIOCli {
  ...
  init(vorpal) {
    debug.wrapOpts(vorpal
      ...
      .option('-f --include-filter <path-regex>', 'only import files match path regex. supply with slashes and do not include collection name; ex: /part\\/.*/i')
      .option('-s --sub-paths <paths>', 'only import containers under specified nested paths. Comma separate')
      .option('-p --collection-path <path>', 'import containers to nested path within the collection')
      ...
      .option('-f --include-filter <path-regex>', 'only download files match path regex. supply with slashes and do not include collection name; ex: /part\\/.*/i')
      .option('-s --sub-paths <paths>', 'only export containers under specified nested paths. Comma separate')
