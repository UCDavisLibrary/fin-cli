...
class HttpCli {
  ...
  async init(vorpal, argv) {
    ...
    this._stdOptionWrapper(
      vorpal.command('http version list [dir]')
            .action(this.getVersions.bind(this))
    );

    this._stdOptionWrapper(
      vorpal.command('http version get <version-name> [dir]')
            .action(this.getVersion.bind(this))
    );

    this._stdOptionWrapper(
      vorpal.command('http version create <version-name> [dir]')
            .action(this.createVersion.bind(this))
    );

    this._stdOptionWrapper(
      vorpal.command('http version revert <version-name> [dir]')
            .action(this.revertToVersion.bind(this))
    );

    this._stdOptionWrapper(
      vorpal.command('http version delete <version-name> [dir]')
  ...
  async getVersions(args) {
    args.dir = location.makeAbsoluteFcPath(args.dir || '.');
