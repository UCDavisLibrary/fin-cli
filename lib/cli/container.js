...
class InteractiveContainer {
  ...
  init(vorpal, location) {
    ...
    vorpal.command('update [fedora_path]')
          .option('-b, --binary', 'Binary file for location')
          .option('-f, --filename', 'Alternative filename for binary, if binary file provided.')
          .option('-r, --rdf', 'Turtle RDF file to describe container.  Will be applied to binary file container if binary provided')
  ...
  async update(args) {
    ...
    var sparql = await this.turtleToSparqlUpdate(newbody, body);
  ...
  async createFromFiles(args) {
    if( args.options.binary ) {
      args.options.binary = this._makeAbsolutePath(args.options.binary);
      if( !fs.existsSync(args.options.binary) ) {
        return Logger.error(`Binary file does not exist: ${args.options.binary}`);
      }
    }
    if( args.options.rdf ) {
      args.options.rdf = this._makeAbsolutePath(args.options.rdf);
    ...
    if( args.options.binary && args.options.rdf) {
      var turtle = fs.readdirSync(args.options.rdf, 'utf-8');
      var sparql = await this.turtleToSparqlUpdate(turtle);
      ...
      console.log('Updated: '+args.fedora_path);
    }
  }
  ...
  _makeAbsolutePath(file) {
    if( !path.isAbsolute(file) ) {
      path.join(os.cwd(), file);
    }
    return file;
  }
  ...
  async turtleToSparqlUpdate(newturtle, oldturtle) {
    ...
    var deletes = '';
    if( del ) {
      del.triples.map(this._genericTripleToSparqlUpdate).join('\n');
    }
