...
class InteractiveLocation {
  ...
  init(vorpal, container) {
    ...
    vorpal.command('ls').action(this.ls.bind(this))
  ...
  async ls(args) {
    ...
    if( fileinfo.type === this.container.TYPES.DATA_FILE ) {
      if( args.data ) return [];
      return console.log(`location ${lspath} is a binary file`);
    }
    ...
    if( body['http://www.w3.org/ns/ldp#contains'] ) {
      ...
      if( args.data ) return [];
      console.log('Invalid directory: '+lspath);
    }
  }
  ...
  async cd(args, command) {
    ...
    args.dir = args.dir+'';
    if( args.dir.match(/^\//) ) this.cwd = args.dir;
    else this.cwd = path.resolve(this.cwd, args.dir);

    var {response, body} = await api.head({path : this.cwd});

    if( response.statusCode !== 200 ) {
       console.log(`Invalid directory: ${this.cwd}`);
       this.cwd = original;
    }
  }
