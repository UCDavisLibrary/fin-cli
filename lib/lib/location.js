...
class Location {
  ...
  async ls(args = {}) {
    ...
    if( fileinfo.type === locationInfo.TYPES.DATA_FILE ) {
      return {
        error : true,
  ...
  /**
   * @method setCwd
   * @description wrapper around LocationStore.setCwd()
   * 
   * @param {String} path
   */
  setCwd(path) {
    LocationStore.setCwd(path);
  }

  /**
   * @method getCwd
   * @description wrapper around LocationStore.getCwd()
   * 
   * @returns {String}
   */
  getCwd() {
    return LocationStore.getCwd();
  }
