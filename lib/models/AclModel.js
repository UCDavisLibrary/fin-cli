...
class AclModel extends ModelBase {
  ...
  constructor() {
    super();
    this.registerIOC('AclModel');
  }

  async acl(args) {
    var tree = await this.walk('/acl', {});
    var displaypath = (args.fedora_path || this.models.LocationModel.cwd()).split('/');
  ...
  async walk(rootdir, tree) {
    ...
    var dirs = await this.models.LocationModel.ls({fedora_path: rootdir, data: true});
    for( var i = 0; i < dirs.length; i++ ) {
      await this.walk(pathutils.joinUrlPath(rootdir, dirs[i]), tree);
    }
